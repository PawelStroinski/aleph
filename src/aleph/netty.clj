  ...
  (when (some? listen-socket)
    (case transport
      (:epoll :kqueue) (when-not (int? listen-socket)
                         (throw (IllegalArgumentException.
                                 (str "With epoll and kqueue transports, only a numeric file descriptor is supported as listen-socket, but received: "
                                      (pr-str listen-socket)))))

      :nio (cond (not (instance? java.nio.channels.ServerSocketChannel listen-socket))
                 (throw (IllegalArgumentException.
                         (str "With NIO transport, only a java.nio.channels.ServerSocketChannel is supported as listen-socket, but received: "
                              (pr-str listen-socket))))
      ...
      (throw (IllegalArgumentException. (str "The listen-socket option is not supported with this transport: "
                                             (pr-str transport)))))))
